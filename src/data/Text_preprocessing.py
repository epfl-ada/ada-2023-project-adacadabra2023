...
def apostrophe(text):
    ap_pattern = re.sub(r'â\x80\x99', "'", text)
    return ap_pattern
...
def cleaning(df, path):
    ...
    if not os.path.exists(main_path):
        ...
        # Converting unicode to apostrophe and expanding contractions
        df["text"] = df["text"].apply(lambda text: apostrophe(text))
        df['text'] = df['text'].apply(contractions.fix)   

        # Removal of puntuacture
        ...
        # Removal of numbers
        df["text"] = df["text"].apply(lambda text: remove_nb(text))
